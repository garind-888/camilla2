#!/usr/bin/env python3
"""
Build script for generating supplements Word document with template replacement.
"""
...
def run_pandoc(input_content: str, output_path: Path, script_dir: Path) -> int:
    ...
    try:
        ...
        pandoc_cmd = [
            ...
            "--reference-doc=" + str(templates_dir / "template.docx"),
            "-o", str(output_path)
        ]
...
def main():
    """Main function to build supplements."""
    # Define paths
    script_dir = Path(__file__).parent
    write_dir = script_dir.parent
    paper_dir = write_dir / "paper"
    output_dir = write_dir / "output"
    output_path = output_dir / "supplements.docx"
    
    # Process supplements file with templates
    supplements_path = paper_dir / "10_supplements.md"
    processed_content = process_file_with_templates(script_dir, supplements_path)
    ...
    config = load_config(script_dir)
    citation_style = config.get("citation_style", "eurointervention.csl")
    print(f"Supplements generated: {output_path}")
    print(f"Citation style used: {citation_style}")
    
    # Open the document automatically
    print("Opening supplements in Microsoft Word...")
    try:
        subprocess.run(["open", "-a", "Microsoft Word", str(output_path)], check=True)
        print("✅ Supplements opened in Microsoft Word!")
    except subprocess.CalledProcessError:
        print("⚠️  Could not open in Microsoft Word, trying default application...")
        try:
            subprocess.run(["open", str(output_path)], check=True)
            print("✅ Supplements opened in default application!")
        except subprocess.CalledProcessError:
            print("⚠️  Could not open supplements automatically. Please open manually.")
    except FileNotFoundError:
